...
namespace Scarecrow
{
    ...
    public class Toggle_SwitchDrawer : MaterialPropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            if (_showList != null && !_simpleShaderGUI.GetShowState(_showList) )
            {
                return;
            }
    ...
    //控件显示控制
    public class SwitchAndShowDrawer : MaterialPropertyDrawer
    {
        //该控件会在以下选项中显示
        private string[] _showList = new string[0];
        //绘制折叠页的ShaderGUI
        ...
        //填写任意数量的选项，当其中一个选项被选中时，该控件会被渲染
        public SwitchAndShowDrawer(string show, params string[] showList)
        {
            _showList = showList;
            _show = show;
            _oldFloat = 1;
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            if (_simpleShaderGUI.GetShowState(_showList) && _simpleShaderGUI.ShowList.Contains(_show))
            {
                editor.DefaultShaderProperty(prop, label);
                if (_waitChange)
                {
    ...
    //图片为空时开始显示
    public class SwitchAndShowByEmptyDrawer : MaterialPropertyDrawer
    {
        //该控件会在以下选项中显示
        private string[] _showList = new string[0];
        //绘制折叠页的ShaderGUI
        ...
        //填写任意数量的选项，当其中一个选项被选中时，该控件会被渲染
        public SwitchAndShowByEmptyDrawer(string show, params string[] showList)
        {
            _showList = showList;
            _show = show;
        }
