...
namespace Scarecrow
{
    ...
    public class FoldoutDrawer : MaterialPropertyDrawer
    {
        ...
        public FoldoutDrawer(float foldoutLevel, float foldoutStyle, float foldoutToggleDraw) : this(foldoutLevel, foldoutStyle, foldoutToggleDraw, 0.0f) { }
        public FoldoutDrawer(float foldoutLevel, float foldoutStyle, float foldoutToggleDraw,float foldoutOpen = 0.0f, params string[] showList)
        {

            //设置折叠页等级
            _foldoutLevel = (int)foldoutLevel;// < 1 ? 1 : level;

            //设置折叠页样式
            switch ((int)foldoutStyle)
            {
                case 2: _foldoutStyle = FoldoutStyle.Median; break;
                case 3: _foldoutStyle = FoldoutStyle.Small; foldoutToggleDraw = 0; break;//如果样式是 小折叠页，则不进行复选框的绘制
                default: _foldoutStyle = FoldoutStyle.Big; break;
            }

            //是否绘制复选框
            _foldoutToggleDraw = (int)foldoutToggleDraw == 0 ? false : true;

            //折叠页默认展开状态
            _foldoutOpen = (int)foldoutOpen == 0 ? false : true;
        ...
        public override void Apply(MaterialProperty prop)
        {
            ...
            {
                if(prop.floatValue != 0 || _foldoutToggleDraw == false)
                {
                    SetFoldoutEditorKeyword(prop, true);
                }
                else
                {                    
                    SetFoldoutEditorKeyword(prop, false);
                }
            }
        }
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
        {
            ...
            //折叠页复选框状态， 非0勾选, 0 不勾选
            _foldoutEditor = (prop.floatValue != 0 || _foldoutToggleDraw == false);
            if(_oldFoldoutEditor != _foldoutEditor)
            {
                _foldoutOpen = _foldoutEditor;
                _oldFoldoutEditor = _foldoutEditor;
            }
