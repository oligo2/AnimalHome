			...
			Pass {
				...
				v2f vert(appdata_t v)
				{
					...
					//OUT.pos = UnityObjectToClipPos(v.vertex);
					   ...
					   OUT.pos = mul(mul(UNITY_MATRIX_P, mvNoTranslation), v.vertex * float4(s, s, s, 1));
	...
	//				for(int i=0; i<int(kSamples); i++)
					{
						...
						float lightAngle = dot(_SunDir, samplePoint) / height;
					...
					if (lightAngle * 2 > 0 && eyeRay.y < 0.0)
					{
						...
						//cIn = lerp(cIn, _Exposure * (cIn * getRayleighPhase(_SunDir, -eyeRay)) + _GroundColor * kInvWavelength, lightAngle * 2);
					}
					//Added
				...
				OUT.groundColor = _Exposure * (cIn * getRayleighPhase(_SunDir, -eyeRay));
				OUT.skyColor = _Exposure * (cIn * getRayleighPhase(_SunDir, -eyeRay));
				...
				// if y < 0 [eyeRay.y > 0] - sky
					...
					if (y < 1.5) //Edited from 0.0
					{
						...
						half mie = calcSunSpot(_SunDir, -ray);
